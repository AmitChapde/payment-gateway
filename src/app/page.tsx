...
export default function HomePage() {
  ...
  const currentTransaction = useAppSelector(
    (state) => state.payment.currentTransaction
  );
  ...
  async function handleRetry() {
    ...
    const updatedTransaction = {
      ...currentTransaction,
      attempts: currentTransaction.attempts + 1,
  ...
  return (
    <main className="min-h-screen bg-zinc-50 px-4 py-10 text-zinc-950 sm:px-6 lg:flex lg:h-screen lg:items-center lg:overflow-hidden lg:px-8 lg:py-6">
      <div className="mx-auto grid w-full max-w-6xl items-stretch gap-8 lg:h-full lg:min-h-0 lg:grid-cols-[minmax(0,28rem)_minmax(0,1fr)] lg:gap-6">
        <div className="space-y-6 lg:min-h-0">
          <PaymentForm />
          <StatusScreen onRetry={handleRetry} />
          <TransactionDetails />
        </div>
        <TransactionHistory />
      </div>
